...
client.on('message', message => { // When the message a message is executed
	...
	if (message.content.toLowerCase() === ('!help')) { // if that message matches ping
		message.reply('```noremakes,shuckle,deod,uturn,jojo```');
       }   
    ...
    if (message.content.toLowerCase()===('!jojo')) { // if that message matches ping
       ...
       message.reply('https://cdn.discordapp.com/attachments/570211056446734349/689243285633499224/ETPcXXCX0AAne8h.png');
    }
    if (message.content.toLowerCase()===('fwqef')) { // if that message matches ping

       message.reply('https://www.youtube.com/watch?v=AkPxbw0QacU');
    }
	}
       });
